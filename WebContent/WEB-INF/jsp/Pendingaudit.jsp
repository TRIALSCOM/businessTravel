    ...
    <div class="col-md-12 col-xs-12 col-lg-12  TSBoxPosition"> 
     ...
     <div class="fh5co-form animate-box TSBox"> 
       ...
       <div class="panel-body"> 
        ...
        <div class="form-group" style="margin-top:15px"> 
         <div class="col-md-6 form-groups"> 
          <label for="txt_search_leaveperson">出行人</label> 
          <input type="text" class="form-control" id="txt_search_leaveperson" /> 
         </div> 
      ...
      <div class="row"> 
       <div class="col-md-12"> 
        <h2>待审核列表</h2> 
        <form id="rejectForm"   action="rejectVerify"  method="post" style="padding: 0; margin: 0;">
          <input id="rejectIds" name="rejectRecordIds"  type="text" style="height: 0;display: none;" />
        </form>
        <form id="acceptForm"   action="acceptVerify"  method="post"  style="padding: 0; margin: 0;">
          <input id="acceptIds"  name="acceptRecordIds"  type="text" style="height: 0;display: none;"/>
        </form>
       <div class="pull-right">
          <button type="button" onclick="rjOptions()" class="btn btn-warning" style="height: 32px;">
            驳回
          </button>
          <button type="button" onclick="acOptions()" class="btn btn-primary" style="height: 32px;">
            审批
          </button>
       </div>
        ...
        <table id="dealHistory" class="table table-striped TSTable table-hover "> 
		 ...
		 <c:forEach  items="${travelRecords}" var="p"> 
          <tr> 
           <td> <input type="checkbox" name="nCheckBoxId" value="${p.id}" /></td> 
           <td>${p.employee.name}</td> 
           <td>${p.travelBeginDate}</td> 
           <td>${p.endPlace}</td> 
           <td>${p.employee.roleInfo.role}</td> 
           <td>${p.allFunding}</td> 
         <!--   estimate_Pay,have_Pay,payment,flie_address,costDescription, -->
           <td> <a  id="watchClick" class="tablelink" onclick="pendingAuditModal('#aduditModal','${p.employee.name}','${p.endPlace}',
                  '${p.travelBeginDate}','${p.daysCount}','${p.applyingFunding}','${p.advanceFunding}','${p.allFunding}','${p.attachment.fileName}','${p.attachment.fileType}','${p.costDescription}','${p.id}')"> 查看</a> 
                  <a  class="tablelink" name="rjClick" >驳回</a> 
                  <a  class="tablelink" name="acClick" >审批</a> </td> 
          </tr> 
          </c:forEach>	  
         </tbody> 
        </table> 
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form id = "deleteOneForm" class="modal-footer">
        ...
        <button type="button"  class="btn btn-primary" onclick="acOpt()" style="height:32px ">
          确认
        </button>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form id = "deleteOneForm" class="modal-footer">
        ...
        <button type="button"  class="btn btn-primary" onclick="rjOpt()" style="height:32px ">
          确认
        </button>
        <input id="deleteOneId" style="display: none"/>
      </form>
...
<!-- 审核模态框（Modal） -->
<div class="modal fade" id="aduditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
     ...
     <div class="modal-footer"> 
      ...
      <button  onclick="rjOpt()" class="btn btn-warning" style="height: 32px;">
          驳回
        </button>
      <button  onclick="acOpt()" class="btn btn-primary" style="height: 32px;">
          审批
        </button>
     </div> 
    </div>
   ...
   function search(){
		...
		var oTxtName=document.getElementById('txt_search_leaveperson');
     ...
     function pendingAuditModal(travel_modal,traverl_name,traverl_place,traverl_trvealtime,traverl_days,estimate_Pay,have_Pay,payment,flie_address,file_type,costDescription, id){
